...
from functions import create_wordcloud, get_text_from_URL, sentiment_analysis
...
# def send_js2(textpath):
#     return send_from_directory(TEXT_DIR, filepath)
...
def upload_file():
    ...
    if request.method == "POST":
        ...
        # Google Language API Sentiment Analysis
        response = sentiment_analysis(text)

        return render_template("index.html", url=url, response=response, text=text, filepath=filepath )

if __name__ == '__main__':
    app.run(debug=True,  host='0.0.0.0', port=5050) # ポートの変更